...
ulgFileName = '202406262124'; % the ulog file name. load 06_28_53 06_45_38  06_54_35 wind 11_39_14  15_57_58 wp:17_09_30, endurance:17_14_37 (18.9) 06_26_32 (17.8) 06_50_20 (19.6) (19.5) 07_10_30 (18.6)
tmp=[ ulgFileName '.mat'];
...
%     fig6=figure(6);
%     subplot(411)
%     plot((actuator_controls(:,1)-actuator_controls(1,1))*1e-6, indi_feedback(:,1),'r-','LineWidth',1);hold on;
%     plot((actuator_controls(:,1)-actuator_controls(1,1))*1e-6, indi_feedback(:,2),'k--','LineWidth',1);hold on;
%     plot((actuator_controls(:,1)-actuator_controls(1,1))*1e-6, indi_feedback(:,3),'b-.','LineWidth',1);hold on;
%     grid on;
%     % axis([-inf inf -0.5 0.5]);
%     title('indi控制量');
%     xlabel({'时间(s)'});
...
%     fig7=figure(7);
%     subplot(411)
%     plot((actuator_controls(:,1)-actuator_controls(1,1))*1e-6, error_feedback(:,1),'r-','LineWidth',1);hold on;
%     plot((actuator_controls(:,1)-actuator_controls(1,1))*1e-6, error_feedback(:,2),'k--','LineWidth',1);hold on;
%     plot((actuator_controls(:,1)-actuator_controls(1,1))*1e-6, error_feedback(:,3),'b-.','LineWidth',1);hold on;
% 
%     grid on;
%     % axis([-inf inf -0.5 0.5]);
%     title('误差反馈控制量');
...
if(isfield(log.data, 'vehicle_angular_acceleration_0'))
    ...
    plot((vehicle_angular_velocity(:,1)-vehicle_angular_velocity(1,1))*1e-6, vehicle_angular_velocity(:,3),'--','LineWidth',1,'color',[0.6,0.2,0,0.5]);hold on;
    grid on;
    % axis([0 1200 -inf inf]);
    title('角加速度');
    xlabel({'时间(s)'});
    ...
    plot((vehicle_angular_velocity(:,1)-vehicle_angular_velocity(1,1))*1e-6, vehicle_angular_velocity(:,4),'--','LineWidth',1,'color',[0.6,0.2,0,0.5]);hold on;
    grid on;
    % axis([0 1200 -inf inf]);
    xlabel({'时间(s)'});
    ylabel('q(rad/s^2)')
    ...
    plot((vehicle_angular_velocity(:,1)-vehicle_angular_velocity(1,1))*1e-6, vehicle_angular_velocity(:,5),'--','LineWidth',1,'color',[0.6,0.2,0,0.5]);hold on;
    grid on;
    % axis([0 1200 -inf inf]);
    xlabel({'时间(s)'});
    ylabel('r(rad/s^2)')
...
if(isfield(log.data, 'vehicle_visual_odometry_0') && isfield(log.data, 'vehicle_attitude_0')) 
    fig16=figure(16);
    plot((vehicle_visual_odometry(:,1)-vehicle_visual_odometry(1,1))*1e-6, visual_odometry_q0,'k-','LineWidth',1);hold on;
    plot((vehicle_attitude(:,1)-vehicle_attitude(1,1))*1e-6, q_0,'r-','LineWidth',1);hold on;grid on;
    % 采样频率不一样，只能粗略估计
    %计算时延方法1
    % % 使用 finddelay 函数
