...
actuator_motors=log.data.actuator_motors_0{:,:};
actuator_outputs=log.data.actuator_outputs_0{:,:};
vehicle_thrust_setpoint=log.data.vehicle_thrust_setpoint_0{:,:};
vehicle_torque_setpoint=log.data.vehicle_torque_setpoint_0{:,:};
actuator_controls_0=log.data.actuator_controls_0_0{:,:};
vehicle_attitude_setpoint=log.data.vehicle_attitude_setpoint_0{:,:};
fig2=figure;

plot((actuator_motors(:,1))*1e-6, actuator_motors(:,3),'k-','LineWidth',1);hold on;
grid on;
% axis([-inf inf -100 100]);
xlabel({'Time(s)'});
ylabel('Angular velocity(deg/s)')
% title('Pitch angular rate');
h=legend('actuator motors');%legend('boxoff');
%% and maybe more figure, all in the variable "log.data"

fig3=figure;

plot((actuator_outputs(:,1))*1e-6, actuator_outputs(:,4),'--','LineWidth',1,'color',[0.6,0.2,0]);hold on;
grid on;
% axis([-inf inf -100 100]);
xlabel({'Time(s)'});
ylabel('Angular velocity(deg/s)')
...
plot((vehicle_attitude_setpoint(:,1))*1e-6, vehicle_attitude_setpoint(:,12),'--','LineWidth',1,'color',[0.6,0.2,0]);hold on;
grid on;
% axis([-inf inf -100 100]);
xlabel({'Time(s)'});
ylabel('vehicle attitude setpoint')
% title('Pitch angular rate');
h=legend('vehicle attitude setpoint');%legend('boxoff');

%% and maybe more figure, all in the variable "log.data"
% rate_error=vehicle_rates_setpoint(:,2:4)-vehicle_angular_velocity(:,3:5);
